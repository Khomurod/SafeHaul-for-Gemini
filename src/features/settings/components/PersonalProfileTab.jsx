...
import { Save, Loader2, Link as LinkIcon, Copy, CheckCircle, Phone } from 'lucide-react';
...
export function PersonalProfileTab({ currentUser, currentCompanyProfile }) {
    ...
    // SMS Assignment State
    const [assignedPhoneNumber, setAssignedPhoneNumber] = useState(null);
    ...
    // Listen for SMS assignment changes
    useEffect(() => {
        if (!currentCompanyProfile?.id || !currentUser?.uid) return;

        const unsub = onSnapshot(
            doc(db, 'companies', currentCompanyProfile.id, 'integrations', 'sms_provider'),
            (snap) => {
                if (snap.exists()) {
                    const data = snap.data();
                    setSmsProvider(data.provider);
                    // Check if current user has an assigned number
                    const assigned = data.assignments?.[currentUser.uid];
        ...
        return () => unsub();
    }, [currentCompanyProfile?.id, currentUser?.uid]);
    ...
    return (
        <div className="space-y-8 max-w-4xl animate-in fade-in">
            ...
            {/* SMS Assignment Display */}
            {assignedPhoneNumber && (
                <div className="border rounded-xl p-6 shadow-sm bg-gradient-to-r from-purple-50 to-violet-50 border-purple-100">
                    <div className="flex items-start gap-4">
                        <div className="p-3 rounded-lg shadow-sm bg-white text-purple-600">
                            <Phone size={24} />
                        </div>
                        <div className="flex-1">
                            <h3 className="text-lg font-bold mb-1 text-purple-900">
                                Your Assigned {smsProvider === '8x8' ? 'Sender ID' : 'Phone Line'}
                            </h3>
                            <p className="text-sm text-purple-700 mb-3">
                                SMS messages you send will appear from this {smsProvider === '8x8' ? 'sender identity' : 'number'}.
                            </p>
                            <div className="flex items-center gap-3">
                                <span className="px-4 py-2 bg-white rounded-lg border border-purple-200 font-mono text-purple-700 font-medium">
                                    {assignedPhoneNumber}
                                </span>

...
export function DashboardBody({
    ...
    const renderCell = (item, colKey, isSelected, isSafeHaul) => {
                ...
                {Array.from({ length: 6 }).map((_, i) => (
                    <tr key={i} className="animate-pulse">
                        ...
                        {visibleColumns.map((colKey, idx) => {
                            const widthStyle = columnWidths[colKey] ? { width: columnWidths[colKey] } : {};
                            return (
                                <td key={idx} className="px-6 py-4" style={widthStyle}>
                                    {colKey === 'name' ? (
            ...
            {data.map(item => {
                ...
                return (
                    <tr
                        ...
                        className={`cursor-pointer transition-colors group border-l-4 ${isSelected
                    ...
                    >
                        ...
                        {visibleColumns.map((colKey) => {
                            ...
                            const alignClass = colConfig.align || 'text-left';
                            const widthStyle = columnWidths[colKey] ? { width: columnWidths[colKey] } : {};

                            return (
                                <td
                                    key={colKey}
                                    className={`px-6 py-4 align-middle overflow-hidden ${alignClass}`}
                                    style={widthStyle}
                                >
                                    {renderCell(item, colKey, isSelected, isSafeHaul)}
                                </td>
                            );
                        })}
                    </tr>

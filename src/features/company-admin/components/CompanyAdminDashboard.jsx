...
// FIXED: Removed direct auth import to rely on Context
...
export function CompanyAdminDashboard() {
  ...
  // FIXED: Ensure hook receives valid user object
  ...
  // FIXED: Replaced direct auth listener with Context effect
  useEffect(() => {
    async function loadUserProfile() {
        if (currentUser) {
            setUserEmail(currentUser.email);
            try {
                const portalUserDoc = await getPortalUser(currentUser.uid);
                ...
                console.warn("Failed to load portal user profile", error);
            }
        }
    }
    loadUserProfile();

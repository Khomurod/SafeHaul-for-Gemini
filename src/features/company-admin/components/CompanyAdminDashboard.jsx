...
// Lazy load potentially heavy details view
const ApplicationDetailView = React.lazy(() => import('./ApplicationDetailView').then(m => ({ default: m.ApplicationDetailView })));
const CampaignsView = React.lazy(() => import('../views/CampaignsView').then(m => ({ default: m.CampaignsView }))); // <--- NEW
...
import {
    ...
    PenTool, MessageSquare // Icon for Documents
} from 'lucide-react';
...
export function CompanyAdminDashboard() {
    ...
    const getActiveIcon = () => {
        switch (dashboard.activeTab) {
            ...
            case 'campaigns': return <MessageSquare size={18} className="text-pink-600" />;
                ...
                <div className="flex-1 overflow-hidden flex flex-col max-w-[1600px] mx-auto w-full p-4 sm:p-6">
                    ...
                    <div className="hidden md:grid md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6 shrink-0">
                        ...
                        <StatCard
                            ...
                            active={dashboard.activeTab === 'campaigns'}
                            colorClass="ring-pink-500 bg-pink-500"
                            onClick={() => dashboard.setActiveTab('campaigns')}
                        />
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden min-h-0 bg-white rounded-xl shadow-sm border border-gray-200">
                        ...
                        {dashboard.activeTab === 'campaigns' ? (
                            <React.Suspense fallback={<div className="p-10 flex justify-center"><div className="animate-spin h-8 w-8 border-b-2 border-pink-600 rounded-full"></div></div>}>
                                <CampaignsView companyId={companyId} />
                            </React.Suspense>
                        ) : (
                            <DashboardTable

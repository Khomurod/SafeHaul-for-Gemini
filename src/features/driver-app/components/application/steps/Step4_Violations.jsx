...
const Step4_Violations = ({ formData, updateFormData, handleFileUpload, onNavigate }) => {
    ...
    const mvrConsentConfig = getConfig('mvrConsent', true);
    const yesNoOptions = YES_NO_OPTIONS;
    const consentMvr = formData['consent-mvr'] === 'yes';
    ...
    return (
        <div id="page-4" className="form-step space-y-6">
            ...
            <fieldset className="border border-gray-300 rounded-lg p-4 space-y-4">
                ...
                {/* MVR Consent Upload (Configurable) */}
                {consentMvr && !mvrConsentConfig.hidden && (
                    <div id="mvr-consent-upload-section" className="space-y-2 pt-4 border-t border-gray-200">
                        <InputField 
                            label="Upload Signed MVR Consent Form" 
                            id="mvr-consent-upload" 

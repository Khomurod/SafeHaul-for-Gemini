...
import { FileSignature, PenTool, CheckCircle, Save, Eraser } from 'lucide-react';
...
const Step9_Consent = ({ formData, updateFormData, onNavigate, onFinalSubmit, handleFileUpload }) => {
    ...
    const [isSigned, setIsSigned] = useState(false); // Local state to track saving
    ...
    const handleSaveSignature = () => {
        ...
        if (dataUrl && dataUrl.length > 500) { // Basic check for non-empty (blank canvas is usually short)
        ...
        <div id="page-9" className="form-step space-y-6">
            ...
            <fieldset className="border border-gray-300 rounded-lg p-4 space-y-4 mt-6 bg-white">
                ...
                <div className="pt-4 border-t border-gray-200 space-y-4">
                    ...
                    {/* Signature Pad Area */}
                    <div className={`relative border-2 border-dashed rounded-lg overflow-hidden h-40 transition-colors ${isSigned ? 'bg-gray-100 border-green-500' : 'bg-white border-blue-200'}`}>
                        <canvas
                            id="signature-canvas"
                            ref={canvasRef}
                            className={`w-full h-full ${isSigned ? 'pointer-events-none opacity-50' : 'cursor-crosshair'}`}
                            style={{ touchAction: 'none' }} // Prevents scrolling while signing
                        ></canvas>
                        
                        {/* Clear Button (Only visible when NOT signed) */}
                        {!isSigned && (
                            <button
                                type="button"
                                onClick={handleClear}
                                className="absolute top-2 right-2 p-2 bg-gray-100/80 hover:bg-gray-200 text-gray-600 rounded-md text-xs font-bold transition-all flex items-center gap-1"
                            >
                        ...
                        {isSigned && (
                            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                <div className="bg-green-100 text-green-800 px-4 py-2 rounded-full font-bold shadow-sm flex items-center gap-2 animate-in fade-in zoom-in">
                                    <CheckCircle size={20} /> Signature Saved
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex justify-end">
                        {!isSigned ? (
                            <button
                                ...
                                className="px-5 py-2 bg-blue-600 text-white text-sm font-bold rounded-lg hover:bg-blue-700 flex items-center gap-2 shadow-sm transition-colors"
                            >
            ...
            <div className="flex justify-between pt-6">
                ...
                <button
                    ...
                    // IMPORTANT: We check 'isSigned' (local state) instead of just formData to prevent race conditions

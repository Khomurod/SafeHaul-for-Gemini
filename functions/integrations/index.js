...
exports.addPhoneLine = onCall({ cors: true }, async (request) => {
    ...
    // Permission Check: Must be Super Admin
    const claims = request.auth.token;
    const isSuperAdmin = claims.globalRole === 'super_admin' || claims.email?.endsWith('@safehaul.io');
    if (!isSuperAdmin) {
        throw new HttpsError('permission-denied', 'Only Super Admins can add phone lines.');
    }
    ...
    try {
        ...
        // 3. Store encrypted JWT in Private Keychain (subcollection)
        const keychainRef = providerDocRef.collection('keychain').doc(sanitizedPhone);
        await keychainRef.set({
...
exports.removePhoneLine = onCall({ cors: true }, async (request) => {
    ...
    const isSuperAdmin = claims.globalRole === 'super_admin' || claims.email?.endsWith('@safehaul.io');
    if (!isSuperAdmin) {
        throw new HttpsError('permission-denied', 'Only Super Admins can remove phone lines.');
    }

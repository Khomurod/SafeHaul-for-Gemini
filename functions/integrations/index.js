...
exports.saveIntegrationConfig = onCall(encryptedCallOptions, async (request) => {
    ...
    // Encrypt sensitive keys before saving, handling __PRESERVE__ markers
    const encryptedConfig = {};
    for (const [key, value] of Object.entries(config)) {
        if (value === '__PRESERVE__') {
            // Keep existing encrypted value - do NOT re-encrypt
            if (existingConfig[key]) {
                encryptedConfig[key] = existingConfig[key];
                console.log(`[saveIntegrationConfig] Preserving existing encrypted value for: ${key}`);
            } else {
                console.warn(`[saveIntegrationConfig] __PRESERVE__ sent for ${key} but no existing value found`);
                // Skip this key entirely - better than encrypting the marker
            }
        } else {

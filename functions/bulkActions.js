        ...
        for (const leadId of batchIds) {
            ...
            try {
                ...
                if (leadSourceType === 'import') {
                    ...
                    else throw new Error("Imported target data missing");
                } else {
                    // Fetch from CRM
                    let leadDocRef;
            ...
            // 5. Safety Delay
            // Skip delay if it's the very last item of the batch to speed up transition? No, keeping it consistent is safer.
            await delay(3000);
        }
...
async function enqueueWorker(companyId, sessionId, delaySeconds) {
    ...
    if (!url) {
        // Fallback to V1 pattern if not explicitly set.
        // NOTE: V2 Cloud Functions have random URL hashes. For V2, you MUST set PROCESS_BULK_BATCH_URL in .env.
